...
def getRandomTrail(k,segNum,centers,maxlen):
    ...
    for i in range(segNum-1,-1,-1):
        if i==segNum-1:
            ...
            for j in range(k):
                for l in range(k):
                    if vis[i+1][l]==1 and getDistence(centers[j][i],centers[l][i+1])<=maxlen:
                        vis[i][j]=1
                        break
                if vis[i][j]==1:
                    ran[i].append(j)
    ...
    for i in range(segNum):
        ...
        while i>0 and maxlen<getDistence(centers[num][i],centers[res[i-1]][i-1]):
            num=random.choice(ran[i])
        res.append(num)
    #print('aaaa',res)
    return res
...
def get_trails_trie(segNum,choiceNum,centers,labels,maxlen):
    ...
    for i in range(choiceNum):
        ...
        for i in range(segNum-1):
            newlen=max(newLen,getDistence(newCenterTrail[i],newCenterTrail[i+1]))
        if newlen<=maxlen:
            myTrie.update(newTrail,1)
            #print(trie.query(newTrail))
    return myTrie

def get_real_trails(myTrie,segNum,choiceNum,centers,labels,epsilon):
    newTrails=[]
    newCnt=0
    newSum=0
    for i in range(choiceNum):
        ...
        if rea>0:
            ...
            newTrails.append(Trail(newCenterTrail,newTrail,rea,\
            max(0,int(rea+np.random.laplace(1/epsilon)))))
    newTrails.sort()
...
def add_virtual_trails(newTrails,newCnt,newSum,centers,segNum,choiceNum,kmeans_k,maxlen,myTrie):
    pos=0
    #print(newSum,choiceNum)
    while newSum<choiceNum:
        if newSum==choiceNum:
            break
        elif pos+1==newCnt-1:
            num=choiceNum-newSum
        else:
            num=random.randint(0,choiceNum-newSum)
        for itera in range(num):
            newTrail=getRandomTrail(kmeans_k,segNum,centers,maxlen)
            #print(newTrail)
            newCenterTrail=[]
