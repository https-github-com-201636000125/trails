...
class Solver:
    ...
    def work(self, E,D):
        ...
        for epsilon in E:
            a = time.time()
            solveTravels.solve(self.segNum, self.choiceNum, self.k,epsilon/(2.0*self.segNum))
            ...
            (myTrie,self.segNum,self.choiceNum,centers,labels,epsilon/(2.0),0,1.5)
            ...
            ReMakeTrails.output_selected_trails(trails,fileName, self.segNum, self.choiceNum)
            
            #发布轨迹
            fileName = 'output/' + 'k(' + str(
                self.k) + ')_e(' + str(epsilon) + ')final_trails' + '.txt'
            ReMakeTrails.output_final_trails(labels,newTrails,fileName,self.segNum,self.choiceNum)
            
            #Hausdorff距离
            fileName = 'output/' + 'k(' + str(
                self.k) + ')_e(' + str(epsilon) + ')Hausdorff' + '.txt'
            sign_Hausdorff=get_sign.get_Hausdorff(newTrails,trails,self.segNum,self.choiceNum)
            ...
            sign_m_info=get_sign.get_m_info(newTrails,labels,trails,centers,maxlen,self.segNum,self.choiceNum)
            ...
            for delta in D:
                ...
                sign_wrq=get_sign.get_wrq(newTrails,trails,delta,self.segNum,self.choiceNum)
